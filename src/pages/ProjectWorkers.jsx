...
const GET_PROJECT_WORKERS = gql`
  query GetProjectWorkers($ProjectId: String!) {
...
function ProjectWorkers() {
  ...
  const [getProjectWorkers, { data: workersData,}] = useLazyQuery(GET_PROJECT_WORKERS);
  ...
  useEffect(() => {
    ...
    .then((response) => {
      ...
      // Fetch project workers using GraphQL query
      getProjectWorkers({ variables: { ProjectId: ID } });
    })
    .catch((error) => {
      console.log(error);
    });
      ...
      <div className='mt-[50px]'>
        <div className='flex items-center justify-between workers__title__wrapper'>
          <h1 className='text-[42px] font-[600] text-TitleColor font-montserrat'>
            {project}
            <span className='font-[500] text-[#83818E] text-[20px] font-montserrat'>
              /Xodimlar
            </span>
          </h1>
        </div>
        <div className='Workers__wrapper'>
            {workersData?.EmployeeProject?.map((i)=>(
          <div className='Workers__card bg-white p-[20px] rounded-[16px]'>
            <div className='flex items-center justify-center flex-col gap-[15px]'>
              <img
                className='w-[105px] h-[105px] rounded-[100px] border-[2px] border-[black]'
                src='' // Assuming you have a source for the image
                alt="foto"
              />
